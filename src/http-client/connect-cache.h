...
static void http_cl_conn_cache_close(nml_http_client *c)
{
	...
	ffstr data = nml_cache_reserve(c->conf->connect.cache, sizeof(struct http_cl_conn_cache_ent));
	...
	if (nml_cache_add(c->conf->connect.cache, name, data))
		goto fail;
...
fail:
	...
	// we own the socket
	nml_cache_free(c->conf->connect.cache, c->connect.cache_data);
}
...
static int http_cl_conn_cache_process(nml_http_client *c)
{
	...
	*(ushort*)(c->connect.cache_name+16) = ffint_be_cpu16(c->resolve.port);
	...
	ffstr data = nml_cache_fetch(c->conf->connect.cache, name);
	if (!data.len)
		return NMLF_DONE;

...
static int xping_attach(struct xping *x)
{
	...
	if (lxdp_attach(&x->lx, x->conf.obj_filename, x->conf.if_index, &conf)) {
		errlog("lxdp_attach: %s", lxdp_error(&x->lx));
		return -1;
	}
	return 0;
}
...
static int xping_xsk_init(struct xping *x)
{
	...
	if (lxdpsk_create(&x->lx, &x->lxsk, x->conf.if_name, 0, &conf)) {
		errlog("lxdpsk_create: %s", lxdp_error(&x->lx));
		return -1;
	}
	if (lxdpsk_enable(&x->lxsk, x->map_xsk)) {
		errlog("lxdpsk_enable: %s", lxdp_error(&x->lx));
		return -1;
	}
	return 0;
}

static int firewall_rule_add(struct xping *x, const struct firewall_rule *rule)
{
	...
	*(ushort*)k.l4_dst_port = ffint_be_cpu16(rule->l4_dst_port);
	...
	if (bpf_map__update_elem(x->map_rules, &k, sizeof(k), &v, sizeof(v), 0)) {
...
static int xping_maps_init(struct xping *x)
{
	static const struct lxdp_map_name maps[] = {
		{ "nml_fw_rule_map",	FF_OFF(struct xping, map_rules) },
		{ "nml_fw_stats_map",	FF_OFF(struct xping, map_stats) },
		{ "nml_fw_xsk_map",		FF_OFF(struct xping, map_xsk) },
		{}
	};
	if (lxdp_maps_link(&x->lx, maps, x)) {
		errlog("lxdp_maps_link", lxdp_error(&x->lx));
		return -1;
	}
...
static int xping_response_handle(lxdpbuf *p)
{
	...
	infolog("%u bytes from %s: icmp_seq=%u ttl=%u time=...ms"
...
static int xping_request_handle(lxdpbuf *p)
{
	...
	infolog("icmp type:%u  id:%u  seq:%u"
		, icmp->type, ffint_be_cpu16_ptr(icmp->echo.id), ffint_be_cpu16_ptr(icmp->echo.seq));
...
static int xping_pkt(struct xping *x, lxdpbuf *p)
{
	...
	if (1) {
		lxdpbuf_shift(p, ffip4_hdr_len(ip));
		return xping_response_handle(p);
	}
...
{
	for (;;) {
		uint n = lxdpsk_read(&x->lxsk, x->buffers, BURST_SIZE);
		if (!n)
			break;
...
{
	uint n = lxdpsk_buf_alloc(&x->lxsk, x->buffers, BURST_SIZE);
	for (uint i = 0;  i < n;  i++) {
...
static int xping_conf_check(struct xping_conf *c)
{
	...
	if (!c->hwsrc.len || !c->hwdst.len
		|| !c->src.len || !c->dst.len) {
		errlog("Please specify hwsrc, hwdst, src and dst");
		return R_BADVAL;
	}
	...
	if (eth_parse(c->hw_src, c->hwsrc.ptr, c->hwsrc.len)
		...
		errlog("Incorrect hwsrc, hwdst, src or dst");
		return R_BADVAL;
	}
...
#define O(m)  (void*)(ffsize)FF_OFF(struct xping_conf, m)
...
static nml_op* xping_create(char **argv)
{
	...
	x->conf.obj_filename = exe->path("mod/nmlfw-xdp-ebpf.o");
	return x;
}

...
static int doh_resolve_process(nml_http_client *c)
{
	...
	if (!(r == 4 || r == 6)) {
		cl_errlog(c, "resolving '%S' via hosts file", &c->conf->host);
		return NMLF_ERR;
	}
...
static int doh_output_open(nml_http_client *c)
{
	...
	d->status = range16_tostr(&c->response.status, c->recv.buf.ptr);
	d->headers = range16_tostr(&c->response.headers, c->recv.buf.ptr);
	return NMLF_OPEN;
}
